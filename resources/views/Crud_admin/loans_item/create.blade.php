                ...
                <div class="card-body">
                    <form action="{{ route('loans_item.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <!-- Create a select with the multiple attribute for multi-selection -->
                            <select class="form-select @error('item_id') is-invalid @enderror" id="item_id"
                                name="item_id[]" multiple>
                                <option selected>-- Pilih Barang --</option>

                                @foreach ($items as $item)
                                    <option value="{{ $item->id }}"
                                        {{ in_array($item->id, old('item_id', [])) ? 'selected' : '' }}>
                                        {{ $item->nama_barang }} (Stok Normal: {{ $item->normal_stock }})
                                    </option>
                                @endforeach
                            </select>
